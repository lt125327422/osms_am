...
import com.itecheasy.core.amazon.vo.GetReportListVO;
 import com.itecheasy.core.amazon.vo.GetReportRequestListResultVO;
 import com.itecheasy.core.amazon.vo.RequestReportResultVO;
import com.itecheasy.core.amazon.vo.GetReportRequestListVO;
...
//            bw.write(""+amazonStockReportVO.getFbaSeaTransit() +"\t");
    ...
    public static RequestReportResultVO requestReport(RequestReportVO requestReportVO,AmazonConfigInfo api) throws MarketplaceWebServiceException {
        ...
        idList.setId(AMAZON_MARKETPLSCEID_LIST);
...
//        }
     ...
     * @return
     */
    public static GetReportRequestListResultVO getReportRequestList(GetReportRequestListVO getReportRequestListVO,AmazonConfigInfo api,int n) throws MarketplaceWebServiceException, InterruptedException {
        MarketplaceWebServiceClient client = getClient(api);
        String AMAZON_SELLER_ID = api.getSellerID();
//        List<String> AMAZON_MARKETPLSCEID_LIST = Arrays.asList(api.getMarketplaceID().split(","));
        ...
        LOGGER.error("getReportRequestList step2:"+api.getTag());
        LOGGER.error("getReportRequestList step2:"+reportRequestList.toXML());
        for (ReportRequestInfo info : infoList) {
            //对象封装回result
            if (!info.getReportProcessingStatus().equals("_DONE_")) {
                if(n > 1 ) {
                    return getReportRequestList(getReportRequestListVO, api,  n-1);
                }
            }
...
//        List<String> AMAZON_MARKETPLSCEID_LIST = Arrays.asList(api.getMarketplaceID().split(","));
        ...
        LOGGER.error("getReportList step3 _reportIdList size :"+reportList.toJSON());
...
//        init(api);
        ...
        LOGGER.error("getReportListByNextToken getReportId: "+response.toJSON());
